...
Installed as the `axiom-engine` console script via `[project.scripts]` in
pyproject.toml. Three subcommands:

    axiom-engine serve          Run the FastAPI HTTP server.
    axiom-engine probe "..."    Send a test query to a running server.
    axiom-engine check-config   Print the resolved Settings (secrets redacted).
"""
...
def _cmd_probe(args: argparse.Namespace) -> int:
    from axiom_engine.cli.probe import run_probe
